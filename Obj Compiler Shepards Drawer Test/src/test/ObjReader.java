...
public class ObjReader {
    ...
    public ObjReader(String directory) throws FileNotFoundException, Exception{
        ...
        for(int i = 0; i < linebreak.size(); i ++){
            ...
            switch(tokens.nextToken()){
                ...
                case "v":{
                    if(temp){
                        vertices.add(new Vertex(Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), name));
                        temp = false;
                    }
                    else vertices.add(new Vertex(Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), vindex));
                    vindex ++;
                    break;
                }
                /*case "vt":{
                ...
                case "vn":{
                    vectors.add(new Vector(Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), Double.parseDouble(tokens.nextToken()), vnindex));
                    if(vnindex < vertices.size()) vertices.get(vnindex-1).setvector(vectors.get(vnindex-1));
                    vnindex ++;
                    break;
                }
                /*case "usemtl":{
                ...
                case "f":{
                    facets.add(new Facet(tokens.nextToken(), tokens.nextToken(), tokens.nextToken(), findex, vertices));
                    findex ++;
                    break;
                }
            }
    ...
    public ArrayList<Facet> compileFacets(){
        return facets;
    }
}